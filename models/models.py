...
#     data['image'] = data['image'].long()
#     if opt.gpu_ids != "-1":
#         data['label'] = data['label'].cuda()
#         data['image'] = data['image'].cuda()
#
#     label_map = data['label']
#     image_map = data['image']
#
#     bs, _, h, w = label_map.size()
#     nc = opt.semantic_nc
#     nc_im = opt.semantic_nc_image
#     if opt.gpu_ids != "-1":
#         input_label = torch.cuda.FloatTensor(bs, nc, h, w).zero_()
#         input_image = torch.cuda.FloatTensor(bs, nc_im, h, w).zero_()
#     else:
#         input_label = torch.FloatTensor(bs, nc, h, w).zero_()
#         input_image = torch.FloatTensor(bs, nc_im, h, w).zero_()
#
#     input_semantics = input_label.scatter_(1, label_map, 1.0)
#     input_semantics_image = input_image.scatter_(1, image_map, 1.0)
#     return input_semantics_image, input_semantics


# I changed Clement's version with the scatter for the image (see above) to the original one (bellow) as the image is a layered tensor here)
def preprocess_input(opt, data):
    data['label'] = data['label'].long()
    if opt.gpu_ids != "-1":
        data['label'] = data['label'].cuda()
    ...
    # for i in range(12):
    #     for j in range(12):
    #         print(f'data_image{i}, {j} (min/max/mean): ', data['image'][i][j].min(), data['image'][i][j].max(), data['image'][i][j].mean())
    return data['image'], input_semantics


def preprocess_label(opt, data):
    data['label'] = data['label'].long()
    if opt.gpu_ids != "-1":
        data['label'] = data['label'].cuda()
